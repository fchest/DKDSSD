...
def eerandtdcf(score_file, label_file, asv_label):
    ...
    nontarget_score_snr_0=[]
    nontarget_score_snr_5=[]
    nontarget_score_snr_10=[]
    nontarget_score_snr_15=[]
    nontarget_score_snr_20=[]
    ...
    print("real: {}  fake: {}".format(count_real, count_spoof))
    ...
    for wav_id in target:
        if snr[wav_id]=="snr0":
            target_score_snr_0.append(float(score[wav_id]))
        elif snr[wav_id]=="snr5":
            target_score_snr_5.append(float(score[wav_id]))
        elif snr[wav_id]=="snr10":
            target_score_snr_10.append(float(score[wav_id]))
        elif snr[wav_id]=="snr15":
            target_score_snr_15.append(float(score[wav_id]))
        elif snr[wav_id]=="snr20":
            target_score_snr_20.append(float(score[wav_id]))           
                     
    for wav_id in nontarget:
        
        if snr[wav_id]=="snr0":
            nontarget_score_snr_0.append(float(score[wav_id]))
        elif snr[wav_id]=="snr5":
            nontarget_score_snr_5.append(float(score[wav_id]))
        elif snr[wav_id]=="snr10":
            nontarget_score_snr_10.append(float(score[wav_id]))
        elif snr[wav_id]=="snr15":
            nontarget_score_snr_15.append(float(score[wav_id]))
        elif snr[wav_id]=="snr20":
            nontarget_score_snr_20.append(float(score[wav_id]))
    
    
  
    target_score_snr_0=np.array(target_score_snr_0)
    target_score_snr_5=np.array(target_score_snr_5)
    target_score_snr_10=np.array(target_score_snr_10)
    target_score_snr_15=np.array(target_score_snr_15)
    target_score_snr_20=np.array(target_score_snr_20)
    
    
    nontarget_score_snr_0=np.array(nontarget_score_snr_0)
    nontarget_score_snr_5=np.array(nontarget_score_snr_5)
    nontarget_score_snr_10=np.array(nontarget_score_snr_10)
    nontarget_score_snr_15=np.array(nontarget_score_snr_15)
    nontarget_score_snr_20=np.array(nontarget_score_snr_20)
    
    eer_cm_0, Threshhold0=compute_eer(target_score_snr_0, nontarget_score_snr_0) 
    eer_cm_5, Threshhold5=compute_eer(target_score_snr_5, nontarget_score_snr_5) 
    eer_cm_10, Threshhold10=compute_eer(target_score_snr_10, nontarget_score_snr_10) 
    eer_cm_15, Threshhold15=compute_eer(target_score_snr_15, nontarget_score_snr_15) 
    eer_cm_20, Threshhold20=compute_eer(target_score_snr_20, nontarget_score_snr_20) 
    ...
    print('\nCM SYSTEM')
    print('   EER            = {:8.5f} Threshhold  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm * 100, Threshhold))
    print('   EER_snr:  0    = {:8.5f} Threshhold0  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm_0 * 100, Threshhold0))
    print('   EER_snr:  5    = {:8.5f} Threshhold5  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm_5 * 100, Threshhold5))
    print('   EER_snr:  10   = {:8.5f} Threshhold10  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm_10 * 100, Threshhold10))
    print('   EER_snr:  15   = {:8.5f} Threshhold15  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm_15 * 100, Threshhold15))
    print('   EER_snr:  20   = {:8.5f} Threshhold20  = {:8.5f} % (Equal error rate for countermeasure)'.format(eer_cm_20 * 100, Threshhold20))

    print('\nTANDEM')
    print('   min-tDCF       = {:8.5f}'.format(min_tDCF))

def main():
    scoreFile="/data4/dingmingming/code_paper/baselines/DKDSSD/Joint_fusion_logit_inter1/test_diffusion_enhanced_2019_random.txt"
    # labelFile="/data4/dingmingming/data_human/ASVspoof_noise/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.seen100random.txt"
    labelFile="/data4/dingmingming/data_human/ASVspoof_noise/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.random.txt"
    # labelFile="/data4/dingmingming/data_human/ASVspoof/ASVspoof2019data_PA/ASVspoof2019_PA_cm_protocols/ASVspoof2019.PA.cm.eval.unseenrandom.txt"

    asvlabel="/data4/dingmingming/data_human/ASVspoof/ASVspoof2019data/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.eval.gi.trl.scores.txt"
